...
<div class="container-fluid">
    <form action="" id="manage-course">
        ...
        <div class="col-lg-6 border-right">
            ...
            <div class="form-group">
                <label for="" class="control-label">Cấp độ</label>
        ...
        <div class="col-lg-6">
            ...
            <div class="row">
                <div class="form-group">
                    <label for="ft" class="control-label">Loại phí</label>
                    <input type="text" id="ft" class="form-control-sm">
                </div>
                <div class="form-group">
                    <label for="" class="control-label">Học phí</label>
                    <input type="number" step="any" min="0" id="amount" class="form-control-sm text-right">
                </div>
                 <div class="form-group pt-1">
                    <label for="" class="control-label">&nbsp;</label>
                    <button class="btn btn-primary btn-sm" type="button" id="add_fee">Thêm</button>
                </div>
            </div>
            <hr>
            <table class="table table-condensed" id="fee-list">
                <thead>
                    <tr>
                        <th width="5%"></th>
                        <th width="50%">Loại phí</th>
                        <th width="45%">Học phí</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
...
<script>
    ...
    $('#add_fee').click(function(){
        ...
        if(amount == '' || ft == ''){
            alert_toast("Vui lòng điền vào trường loại phí và học phí trước.",'warning')
            return false;
        }
    ...
    $('#manage-course').submit(function(e){
        ...
        if($('#fee-list tbody').find('[name="fid[]"]').length <= 0){
            alert_toast("Vui lòng chèn ít nhất 1 hàng trong bảng phí",'danger')
            end_load()
            return false;
        }
        $.ajax({
            ...
            success:function(resp){
                ...
                $('#msg').html('<div class="alert alert-danger mx-2">Tên và cấp độ khóa học đã tồn tại .</div>')
                end_load()
                }   
            }
        })
